...
def update_existing_decorator(class_info: Dict) -> str:
    ...
    # Parse existing decorator content carefully
    ...
    # Combine existing and new content carefully
    if existing_content:
        # Ensure existing content ends with comma if it doesn't already
...
def apply_updates_to_file(file_path: str, updates: List[Dict]) -> int:
    ...
    for update in updates:
        ...
        if new_decorator == update["decorator_text"]:
            continue  # No change needed

        # Find the full decorator + class line combination
...
def main():
    ...
    print("🚀 Starting robust metadata transfer completion (v2)...")
