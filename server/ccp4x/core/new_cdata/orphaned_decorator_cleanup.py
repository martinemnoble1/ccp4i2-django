...
def find_and_remove_orphaned_decorators(file_path):
    ...
    valid_matches = list(re.finditer(r'@cdata_class\s*\([^@]*?\)\s*class', content, re.DOTALL))
...
def main():
    ...
    for file_path in py_files:
        ...
        pattern_count = len(re.findall(r'@cdata_class\s*\([^@]*?\)\s*class', content, re.DOTALL))
        
        if simple_count != pattern_count:
            print(f"  ⚠️  {file_path}: Still has {simple_count - pattern_count} orphaned decorators")
        else:
