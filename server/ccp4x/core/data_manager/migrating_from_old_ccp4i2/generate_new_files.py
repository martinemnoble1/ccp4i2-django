...
def topo_sort_classes(class_list):
    ...
    name_to_data = {name: data for name, data in class_list}
    ...
    while no_deps:
        n = no_deps.pop()
        sorted_classes.append((n, name_to_data[n]))
        for m in graph:
            if n in graph[m]:
                graph[m].remove(n)
        graph.pop(n)
    ...
    if graph:
        ...
        for name in graph:
            sorted_classes.append((name, name_to_data[name]))
...
def main():
    ...
    out_dir = "OUTPUT_DIRECTORY"  # Set your output directory here
    ...
    for name, class_data in classes.items():
        ...
        if name in ["CDataFile", "CDataFileContent", "CXmlDataFile", "CI2XmlDataFile"]:
            basename = "CCP4BaseFile.py"
        if basename not in file_class_map:
            file_class_map[basename] = []
        print("Sorting classes for file:", basename)
        sorted_class_data = topo_sort_classes(class_data)
        file_class_map[basename].append((name, sorted_class_data))
        ...
        for basename, class_list in file_class_map.items():
            ...
            with open(out_path, "w", encoding="utf-8") as f:
                ...
                for name, class_data in sorted_class_list:
                    f.write(render_class(name, class_data))
                    f.write("\n")
            # Format the file with autopep8 for PEP8 compliance
            try:
                subprocess.run(["autopep8", "--in-place", out_path], check=True)
