...
class ParamsXmlHandler:
    ...
    def export_params_xml(self, task: CData, output_path: str, user_id: str = None) -> bool:
        """
        ...
        try:
            ...
            ET.SubElement(header, "userId").text = user_id or os.getenv('USER', 'unknown')
            ET.SubElement(header, "hostName").text = socket.gethostname()
            ET.SubElement(header, "creationTime").text = datetime.now().strftime("%H:%M %d/%b/%y")
            ET.SubElement(header, "ccp4iVersion").text = "alpha_rev_90011"  # Could be configurable
            ET.SubElement(header, "pluginName").text = getattr(task, 'name', 'unknown_task')
    ...
    def _export_container(self, container: CData, parent_elem: ET.Element):
        ...
        for attr_name in sorted(dir(container)):
            if (not attr_name.startswith('_') and 
                ...
                attr_name not in ['child_added', 'child_removed', 'destroyed', 
    ...
    def _export_file_data(self, file_obj: CData, elem: ET.Element):
        ...
        file_attrs = ['project', 'baseName', 'relPath', 'annotation', 'dbFileId', 'subType', 'contentFlag']
    ...
    def _import_container_values(self, xml_container: ET.Element, cdata_container: CData) -> int:
