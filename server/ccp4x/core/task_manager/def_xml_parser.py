...
class DefXmlParser:
        ...
        try:
            ...
            registry.update(
                {
                    ...
                    "CString": CString,
                    "CIntRange": CIntRange,
                    "CFloatRange": CFloatRange,
                    "CContainer": CContainer,
                    "CList": CList,
                }
            )

            # Add all other generated classes
            import importlib
            import pkgutil
            ...
            try:
                ...
                for importer, modname, ispkg in pkgutil.iter_modules(
                    ...
                    if not modname.endswith("_classes"):
                        continue
                ...
                pass  # Generated classes not available yet

            return registry

        except ImportError as e:
            print(f"Warning: Could not import generated classes: {e}")
            return {
                "CInt": CInt,
                "CFloat": CFloat,
                "CBoolean": CBoolean,
    ...
    def _create_list_object(
        self, qualifiers: Dict[str, Any], content: ET.Element
    ) -> "CList":
        """Create a CList object with proper item type."""
        # We need to implement CList - for now create a simple version
        try:
            from .new_cdata.fundamental_types import CList

            return CList()
        except ImportError:
            # Create a basic list-like object
            class CList(CData):
                def __init__(self):
                    super().__init__()
                    self._items = []
                    ...
                    try:
                        if hasattr(obj, "_set_default_value"):
                            ...
                            if hasattr(obj, "value"):
                                obj.value = value
                                if hasattr(obj, "_value_states") and hasattr(
                                    obj, "ValueState"
                                ):
                                    obj._value_states["value"] = obj.ValueState.DEFAULT
                    except Exception as e:
                        print(
