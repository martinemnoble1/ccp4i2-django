...
def demonstrate_complete_workflow():
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.def.xml', delete=False) as def_file:
        def_file.write(SAMPLE_DEF_XML)
        def_xml_path = def_file.name
    
    with tempfile.NamedTemporaryFile(mode='w', suffix='.params.xml', delete=False) as params_file:
    ...
    try:
        ...
        params_to_check = ['DATA_METHOD', 'ADD_WATERS', 'NCYCLES', 'WEIGHT', 'B_REFINEMENT_MODE', 'OCCUPANCY_REFINEMENT']
            ...
            try:
                ...
                if obj and hasattr(obj, 'value'):
                    ...
                    if actual_value == expected_value:
                        ...
                        print(f"  ❌ {path}: {actual_value} (expected: {expected_value})")
                        all_correct = False
                else:
        ...
        print(f"  {'✅' if all_correct else '❌'} Round-trip verification: {'All values correct' if all_correct else 'Some values incorrect'}")
        ...
        if all_correct:
            ...
            print("  • Handle multiple data types (strings, booleans, integers, floats)")
